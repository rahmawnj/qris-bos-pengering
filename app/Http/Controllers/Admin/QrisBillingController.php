...
class QrisBillingController extends Controller
{
    ...
    public function report(Request $request) { $period=$request->input('period'); $ownerId=$request->input('owner_id'); $search=$request->input('search'); $payments=QrisBillingPayment::with(['outlet.owner.user','outlet.devices'])->where('status','paid')->when($period,fn($q)=>$q->whereRaw("DATE_FORMAT(period_end, '%Y-%m') = ?",[$period]))->whereHas('outlet',function($q)use($ownerId,$search){$q->when($ownerId,fn($x)=>$x->where('owner_id',$ownerId))->when($search,function($x)use($search){$x->where(function($i)use($search){$i->where('outlet_name','like',"%{$search}%")->orWhere('code','like',"%{$search}%")->orWhereHas('owner',function($o)use($search){$o->where('brand_name','like',"%{$search}%")->orWhere('name','like',"%{$search}%");});});});})->latest('paid_at')->get(); $summary=['total'=>$payments->count(),'paid'=>$payments->count(),'pending'=>0,'due'=>0,'amount'=>$payments->sum('amount'),'paid_amount'=>$payments->sum('amount'),'unpaid_amount'=>0]; $owners=Owner::orderBy('brand_name')->get(); $status='paid'; return view('admin.qris_billing.report',compact('payments','period','status','ownerId','search','summary','owners')); }
