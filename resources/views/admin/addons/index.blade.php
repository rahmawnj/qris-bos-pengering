    ...
    <div class="panel panel-inverse">
        <div class="panel-heading">
            ...
            <div class="panel-heading-btn">
                ...
                <a href="{{ route('admin.addons.create') }}" class="btn btn-xs btn-primary"><i class="fa fa-plus"></i> Tambah</a>
            </div>
        </div>
                        ...
                        @foreach ($addons as $addon)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div style="font-size: 0.9em; color: gray;">{{ $addon->outlet->outlet_name ?? 'N/A Outlet' }}</div>
                                </td>
                                <td>{{ $addon->name }}</td>
                                <td>{{ $addon->category ?? '-' }}</td> {{-- Display category, default to '-' if null --}}
                                <td>Rp {{ number_format($addon->price, 2, ',', '.') }}</td>
                                <td>
                                    @if ($addon->is_active)
            ...
            $('#data-table').DataTable({
                ...
                buttons: fullExportButtons({
                    ...
                    columns: [
                        ...
                        { title: 'Outlet', data: 'outlet', source: 1 },
                        { title: 'Nama Add-on', data: 'name', source: 2 },
                        { title: 'Kategori', data: 'category', source: 3 },
                        { title: 'Harga', data: 'price', source: 4 },
                        { title: 'Status', data: 'status', source: 5 }
                    ]
                }),
            });
