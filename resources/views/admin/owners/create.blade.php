            ...
            <form action="{{ route('admin.owners.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3 text-center">
                    <img id="brandLogoPreview" src="{{ asset('assets/img/default-img.png') }}"
                        style=" height: 80px; cursor: pointer; border-radius: 10%; object-fit: cover;"
                        onclick="document.getElementById('brand_logo').click();">
                    <input type="file" class="d-none @error('brand_logo') is-invalid @enderror" id="brand_logo"
                        name="brand_logo" accept="image/*" onchange="previewBrandLogo(event)">
                    <button type="button" class="btn btn-xs btn-purple btn-secondary mt-2"
                        onclick="document.getElementById('brand_logo').click();">Ganti Logo</button>
                    @error('brand_logo')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="email" class="form-control @error('brand_email') is-invalid @enderror" id="brand_email"
                        name="brand_email" value="{{ old('brand_email') }}" required>
                    @error('brand_email')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('payment_account_type') is-invalid @enderror" id="payment_account_type" name="payment_account_type" required onchange="toggleMerchantId()">
                        <option value="general" {{ old('payment_account_type') === 'general' ? 'selected' : '' }}>General (Bos Pengering)</option>
                        <option value="owner" {{ old('payment_account_type') === 'owner' ? 'selected' : '' }}>Owner (Akun Sendiri)</option>
                    </select>
                ...
                <div class="mb-3" id="merchant_id_group">
                    ...
                    <input type="text" class="form-control @error('merchant_id') is-invalid @enderror" id="merchant_id"
                        name="merchant_id" value="{{ old('merchant_id') }}" maxlength="100">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('withdrawal_fee_charged') is-invalid @enderror" id="withdrawal_fee_charged" name="withdrawal_fee_charged" required>
                        <option value="1" {{ old('withdrawal_fee_charged', true) == '1' ? 'selected' : '' }}>Ya, dikenakan</option>
                        <option value="0" {{ old('withdrawal_fee_charged', true) == '0' ? 'selected' : '' }}>Tidak, tidak dikenakan</option>
                    </select>
                ...
                <a href="{{ route('admin.owners.index') }}" class="btn btn-default">Cancel</a>
            </form>
